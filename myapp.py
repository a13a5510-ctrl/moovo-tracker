...
spy_log(f"[System] 偵察啟動: {current_time_tp.strftime('%Y-%m-%d %H:%M:%S')}")
...
async def main():
    ...
    if raw_data:
        ...
        prompt = f"""
你現在是商業分析特工。請用冷酷口吻撰寫 Moovo 監測報告。
🕵️ 指令：
1. **極簡化**：每站限一行。標註站名、台數、變動、冷區計數。
2. **格式**：[🧠 AI深度] 時間:{time_str}。
3. **判讀**：diff!=0 ⚡活躍；cold_count>=3 🚨癱瘓。
4. **語言**：必須全部使用繁體中文。嚴禁提到 YouBike。
情報：{full_report}
"""
        ...
        if final_msg:
            ...
            # 🎯 📡 備援模式：極簡繁體版
            spy_log("[System] AI 離線，發送極簡備援報告...")
            backup = f"[📡 原始情報] 時間 {time_str}\n"
